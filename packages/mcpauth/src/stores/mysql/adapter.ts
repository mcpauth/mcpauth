...
export function createMysqlAdapter(db: Kysely<DB>): GenericAdapter {
  return {
    async create({ model, data }) {
      const tableName = modelToTableMap[model as ModelName];
      await db.insertInto(tableName).values(data as any).execute();
      // MySQL insert doesn't return the created row by default.
      // The storage layer is responsible for the data, so we return the input data.
      return data;
    },
    ...
    async update({ model, where, data }) {
      ...
      // Fetch and return the updated record, as MySQL doesn't return it.
